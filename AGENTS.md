...
`neon-branch-cleanup.yml` reaps the Neon preview database for a pull request when that pull
request closes, and sweeps weekly for any it missed. It is seeded here so a project that later
adopts Neon is already carrying it. Neon's Vercel integration deletes a `preview/<git-branch>`
database only when the Vercel *deployment* is removed — six months by default — so merging a
pull request and deleting the git branch leaves the database billing at about $1.50/month
indefinitely; pathfinder reached 93 branches and $79.60 of an $81.78 invoice that way. The
workflow distinguishes three states: no `NEON_PROJECT_ID` variable means the repo is not a Neon
project and every job skips, `NEON_PROJECT_ID` without the `NEON_API_KEY` secret fails with exit
78 because a half-configured reaper deletes nothing while appearing healthy, and both present
reaps. It runs on `pull_request_target` rather than `pull_request` because a Dependabot-authored
`pull_request` run reads only Dependabot secrets and would resolve `NEON_API_KEY` to an empty
string, silently reaping nothing on a large share of pull requests; it never checks out or
executes pull request code, and the head ref reaches the shell only through `env:`. It gates
nothing and is never a required check.
